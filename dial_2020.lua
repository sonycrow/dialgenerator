...
-- Load
function onLoad()
    self.createButton({click_function = "create",  function_owner = self, label = '██', position = {-0.2, 0.15, -0.05}, rotation = {0, 15, 0}, scale = {0.1, 0.1, 0.1}, width = 800, height = 800, font_size = 400, tooltip = "Create Dial"})
end
...
function create()
    ...
    if (description ~= "") then
        local url = _DIALTEMPLATE .. enc(description)
        for i = 1, 12 do
            if (i > 1) then
                dial = dial.setState(i)
            end

            dial.setCustomObject({ diffuse = url})
        end

        dial = dial.setState(1)
        dial.setDescription("")
        dial.setName("")
        dial.reload()
    end
